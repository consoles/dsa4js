from typing import List

class Solution:
    def largestDivisibleSubset(self, nums: List[int]) -> List[int]:
        # 暴力法：
        # 1. 求出所有的子集
        # 2. 对子集进行检查
        n = len(nums)
        subsets = []
        def build_subset(index: int, path: List[int]):
            if index == n:
                subsets.append(path[::])
                return
            # 不选当前元素
            build_subset(index + 1, path)
            # 选当前元素
            path.append(nums[index])
            build_subset(index + 1, path)
            path.pop()

        def check_subset_divisible(subset: List[int]) -> bool:
            n = len(subset)
            for i in range(n):
                for j in range(i+1, n):
                    if i == j:
                        continue
                    if subset[i] % subset[j] != 0 and subset[j] % subset[i] != 0:
                        return False
            return True        

        build_subset(0, [])

        max_subset_len = 0
        max_subset = []
        for subset in subsets:
            if len(subset) > max_subset_len:
                ok = check_subset_divisible(subset)
                if ok:
                    max_subset_len = len(subset)
                    max_subset = subset

        return max_subset

    def largestDivisibleSubset2(self, nums: List[int]) -> List[int]:     
        # 方法 1 的暴力法在 24/29 用例会报内存超限
        # 问题可能在于生成子集的方法，改成生成器
        # 改成生成器之后又超时了，WTF
        # 这个测试用例有 638 个元素，子集的格式有 2^638 天文数字了，无法穷尽子集
        n = len(nums)
        def gen_subset(i: int, path: List[int]):
            if i == n:
                yield path[::]
                return
            yield from gen_subset(i + 1, path)
            path.append(nums[i])
            yield from gen_subset(i + 1, path)
            path.pop()

        def check_subset_divisible(subset: List[int]) -> bool:
            n = len(subset)
            for i in range(n):
                for j in range(i+1, n):
                    if i == j:
                        continue
                    if subset[i] % subset[j] != 0 and subset[j] % subset[i] != 0:
                        return False
            return True  
        
        g = gen_subset(0, [])
        max_subset_len = 0
        max_subset = []
        print(n)
        for subset in g:
            if len(subset) > max_subset_len:
                ok = check_subset_divisible(subset)
                if ok:
                    max_subset_len = len(subset)
                    max_subset = subset
        return max_subset

    def largestDivisibleSubset3(self, nums: List[int]) -> List[int]:
        # 1. 排序数组
        # 确保后续检查的时候 nums[i] 只可能被前面的数字整除(nums[i] % nums[j] == 0，其中 i > j)

        # 2. dp 表格设计
        # dp[i] 表示以 nums[i] 为最大元素的最大可整除子集的大小
        # parent[i] 记录子集 nums[i] 的前驱元素索引（用于回溯路径）
        # 初始化：dp[i] = 1（至少包含自身），parent[i] = -1（无前驱）

        # 3. 填充 dp 表
        # 对于每个 nums[i]，检查所有 j < i 的数：
        # 如果 nums[i] % nums[j] == 0 且 dp[j] + 1 > dp[i]
        #   a. 更新 dp[i] = dp[j] + 1
        #   b. 记录 parent[i] = j

        # 4. 回溯最大子集
        # 找到 dp 表中的最大值及其索引 max_idx
        # 从 nums[max_idx] 开始，通过 parent 数组回溯路径，构造最终子集

        nums.sort()  # 升序排序
        n = len(nums)
        dp = [1] * n      # dp[i] 记录以 nums[i] 结尾的最大子集大小
        parent = [-1] * n  # 记录前驱索引
        
        max_size = 1
        max_idx = 0
        
        for i in range(1, n):
            for j in range(i):
                if nums[i] % nums[j] == 0 and dp[j] + 1 > dp[i]:
                    dp[i] = dp[j] + 1
                    parent[i] = j
                    if dp[i] > max_size:
                        max_size = dp[i]
                        max_idx = i
        
        # 回溯构造结果
        result = []
        idx = max_idx
        while idx != -1:
            result.append(nums[idx])
            idx = parent[idx]
        return result[::-1]

    def largestDivisibleSubset4(self, nums: List[int]) -> List[int]:
        # https://leetcode.cn/problems/largest-divisible-subset/solutions/738882/gong-shui-san-xie-noxiang-xin-ke-xue-xi-0a3jc
        # 最长上升子序列的变形题目

        # 某个状态的转移依赖取决于前一个状态的关系
        # 即 nums[i] 能否接在 nums[j] 后面，取决于是否满足 nums[i] % nums[j] == 0

        # 定义 f[i] 为考虑前 i 个数字，且以第 i 个数为结尾的最长【整除子集】的长度
        # 我们不失一般性地考虑任意位置 i，存在下面 2 种情况：
        # 1. 如果在 i 之前找不到符合条件 nums[i] % nums[j] 的位置 j，那么 nums[i] 不能接在位置 i 之前的任何数的后面，只能自己作为【整除子集】的第一个数，此时状态转移方程为 f[i] = 1
        # 2. 如果在 i 之前能够找到符合条件的位置 j，则取所有符合条件的 f[j] 的最大值，代表如果希望找到以 nums[i] 为结尾的最长【整除子集】，需要将 nums[i] 接到符合条件的最长的 nums[j] 后面，此时状态转移方程为 f[i] = f[j] + 1

        # 同时我们需要求解出具体的方案，需要使用额外的 g[] 数组来记录每个状态到底是由哪个状态转移而来
        # 定义 g[i] 为记录 f[i] 是由哪个下标的状态转移而来，如果 f[i] = fj[j] + 1，则有 g[i] = j
        # 当我们求解到所有的状态值之后，可以对 g[] 进行遍历，取得具体的最长【整数子集】长度和对饮的下标，然后使用 g[] 数组进行回溯（题目要求了无重复整数）

        # 这个解法能成立，问题能转化成【最长上升子序列 LIS】问题进行求解的本质是利用了【全序关系】中的【可传递性】
        # 在 LIS 中我们利用了 >= 的传递性，因此当我们某个数 a 能够接在 b 后面，只需要确保 a >= b 成立，即可确保 a 大于等于 b 之前的所有值
        # 在这个问题中 【倍数/约数关系】具有传递性
        # 由于我们将 nums[i] 往某个数字后面接时（假设为 nums[j]），只检查了其与 nums[j] 的关系，并没有去检查 nums[i] 与 nums[j] 之前的数值是否具有「倍数/约数关系」
        # 换句话说，我们只确保了最终答案 [a1, a2, a3, ..., an] 相邻两数值之间具有「倍数/约数关系」，并不明确任意两值之间具有「倍数/约数关系」
        # 因此需要证得由 a∣b 和 b∣c，可推导出 a∣c 的传递性
        # 由 a∣b 可得 b=x∗a
        # 由 b∣c 可得 c=y∗b
        # 最终有 c=y∗b=y∗x∗a，由于 x 和 y 都是整数，因此可得 a∣c
        # 得证「倍数/约数关系」具有传递性
        nums.sort()
        n = len(nums)
        f, g = [0] * n, [0] * n
        for i in range(n):
            # 至少包含自身一个数，因此起始长度为 1，由自身转移而来
            length, prev = 1, i
            for j in range(i):
                if nums[i] % nums[j] == 0:
                    # 如果能接到更长的序列后面，则更新【最大长度】和【从何处转移而来】
                    if f[j] + 1 > length:
                        length = f[j] + 1
                        prev = j
            # 记录【最终长度】以及【从何处转移而来】
            f[i] = length
            g[i] = prev
            
        # 遍历所有的 f[i]，取得【最大长度】和【对应下标】
        max_len = idx = -1
        for i in range(n):
            if f[i] > max_len:
                idx = i
                max_len = f[i]
        
        # 使用 g[] 数组回溯出具体方案
        ans = []
        while len(ans) < max_len:
            ans.append(nums[idx])
            idx = g[idx]
        ans.reverse()
        return ans

# [1,2] or [1,3]
# nums = [1,2,3]

# [1,2,4,8]
# nums = [1,2,4,8]

nums = [472,105,328,922,963,625,827,232,913,832,730,152,697,614,570,639,459,895,70,488,239,596,930,402,904,197,911,126,876,535,597,332,863,870,646,839,878,751,528,959,929,679,7,543,248,353,494,67,841,732,662,122,485,565,318,747,42,545,282,638,228,496,860,874,334,556,103,437,83,61,365,761,259,361,64,612,206,131,452,938,144,573,774,243,521,102,787,154,92,15,649,951,240,506,342,770,940,135,700,820,393,412,884,356,618,366,807,907,861,380,32,921,622,23,760,57,466,416,391,273,706,512,212,998,263,279,743,27,514,98,996,231,905,902,91,636,285,364,609,189,738,368,203,846,6,970,210,748,594,268,157,726,257,419,999,797,869,96,792,451,352,693,894,987,978,595,518,346,315,114,900,160,119,721,218,814,988,477,66,733,740,421,537,642,178,882,21,337,868,350,945,815,219,290,129,684,640,155,409,712,372,222,848,765,932,283,14,819,544,146,425,958,644,903,458,473,85,627,845,641,340,198,961,456,992,495,413,170,390,559,143,166,428,100,809,647,354,619,435,250,837,533,530,883,948,816,432,261,623,560,503,406,916,147,826,277,312,253,729,251,434,443,686,683,944,186,829,898,63,457,405,417,811,579,957,404,532,621,8,260,445,45,887,296,728,236,591,447,422,696,481,893,264,974,701,714,379,12,482,541,942,385,849,345,469,705,527,571,799,962,650,387,552,97,808,971,242,803,36,505,777,89,507,52,47,436,703,716,794,229,628,108,800,779,69,449,165,973,897,324,943,689,917,745,76,873,164,616,9,24,237,739,892,818,718,351,617,420,744,314,687,956,782,781,297,424,737,912,522,272,408,369,645,275,611,840,382,125,890,162,568,295,349,301,997,38,908,179,333,439,899,329,65,159,142,121,923,247,854,802,330,335,43,843,254,137,954,515,790,950,566,857,490,822,519,286,150,583,796,291,941,37,742,723,784,976,374,780,888,569,879,805,88,468,463,969,258,877,176,564,654,798,789,793,401,493,557,592,202,418,115,852,462,953,717,338,480,981,396,634,836,681,629,935,553,540,397,665,758,817,224,821,158,968,323,17,3,776,177,81,906,749,344,169,140,926,118,501,927,504,598,875,34,643,548,813,470,508,736,602,378,59,652,994,915,249,213,661,450,39,267,586,55,727,25,113,141,734,972,872,300,516,526,270,112,555,171,766,256,127,10,316,554,194,448,657,847,174,241,620,68,562,426,133,265,491,909,989,168,173,167,928,41,567,986,724,658,208,699,709,306,933,2,653,881,298,750,577,702,319,694,394,601,651,309,16,310,668,486,192,136,33,856,134,563,370,358,498,593,230,484,308,851,235,51,551,536,980,415,810,246,307,79,221,580,576,101,438,791,455,673,75,610,376,44,624,399,294,271,195,725,637,78,773,806,967,163,871,389,453,558,925,193,467,274,371,788,599]
r = Solution().largestDivisibleSubset3(nums)
print(r)
