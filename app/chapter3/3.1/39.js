...
function wordCount(arr) {
  ...
  const sts = [require('../SequentialSearchST'), require('../BinarySearchST')].map(ClZ => new ClZ());
  ...
  fs.writeFileSync(path.join(__dirname, '39.g2.data.json'), JSON.stringify(data));
}
...
// 同样的数据，同样的算法，在mbp上二分搜索快，但是在内网服务器上反而顺序查找快
